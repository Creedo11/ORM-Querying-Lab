...
def problem_one(request):
  ...
  students_above_3gpa = Student.objects.filter(gpa__gt=3.0).order_by("-gpa")

  for student in students_above_3gpa:
...
def problem_two(request):
  ...
  instructors_prior_to_2010 = Instructor.objects.filter(hire_date__lt="2010-01-01")
...
def problem_six(request):
    ...
    student_id = 11
    update_student = Student.objects.filter(id="11").update(gpa="3.5")
...
def bonus_problem(request):
  ...
  # -___- (I've struggled long enough on this! I know it isn't that hard smh.)

  for instructor in instructors_with_one_course:
    print(f"Instructor Name: {instructor.first_name} {instructor.last_name}")
